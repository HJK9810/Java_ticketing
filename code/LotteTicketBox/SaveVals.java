	...
	protected void inputFile() { // 파일에 입력한 값들 이어쓰기
		...
		try {
			...
			for (OrderData item : orderList) { // forEach 사용
				...
				else if (item.getTicketType() == StaticValue.getParkTicket()) str.append(String.format("%-7s,", "파크이용권"));
				// 권종
				...
				// 연령
				if (item.getAge() == StaticValue.getOld()) str.append(String.format("%-4s,", "노인"));
				else if (item.getAge() == StaticValue.getAdult()) str.append(String.format("%-4s,", "어른")); // 어른
				else if (item.getAge() == StaticValue.getTeen()) str.append(String.format("%-4s,", "청소년")); // 청소년
				else if (item.getAge() == StaticValue.getChild()) str.append(String.format("%-4s,", "어린이")); // 어린이
				else str.append(String.format("%-4s,", "베이비"));
				// 수량 & 가격
				str.append(String.format("%d,%d,", item.getOrderCount(), item.getSum()));
				// 우대사항
				if (item.getAdventageType() == StaticValue.getNone()) str.append("없음\n");
				else if (item.getAdventageType() == StaticValue.getDisable()) str.append("장애인\n");
